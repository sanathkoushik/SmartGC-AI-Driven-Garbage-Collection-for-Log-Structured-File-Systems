...
def write_final_readme(ablation: pd.DataFrame, model_comparison: pd.DataFrame) -> None:
    """Summarise what the pipeline actually produced, using measured values only.

    Regenerated on every analyse run, so it cannot drift from the CSVs beside it.
    A section whose input is missing says so rather than being silently omitted.
    """
    FINAL_DIR.mkdir(parents=True, exist_ok=True)
    manifest_path = FINAL_DIR / "experiment_manifest.json"
    manifest = (json.loads(manifest_path.read_text(encoding="utf-8"))
                if manifest_path.is_file() else {})
    roles = manifest.get("workload_roles") or {}
    environment = manifest.get("environment") or {}

    lines: list[str] = []
    add = lines.append

    add("# SmartGC final results")
    add("")
    add("Generated by `python experiments/analyze_results.py`. Every value below is")
    add("read from a CSV in this directory; nothing is transcribed by hand.")
    add("")
    if manifest:
        add(f"* Git commit: `{environment.get('git_commit', 'unknown')}`")
        add(f"* Seed: `{manifest.get('seed')}`")
        add(f"* Python {environment.get('python_version')}, "
            f"PyTorch {environment.get('torch_version')} on {environment.get('torch_device')}")
        if isinstance(manifest.get("max_sequences_per_trace"), int):
            add(f"* Sequences per trace (chronological prefix): "
                f"{manifest['max_sequences_per_trace']:,}")
        if isinstance(manifest.get("max_writes_per_run"), int):
            add(f"* Writes replayed per simulation: {manifest['max_writes_per_run']:,}")
        add(f"* Generated: {manifest.get('generated_at_utc')}")
        add("")

    add("## The pipeline")
    add("")
    add("```")
    add("Real block-I/O trace (MSR Cambridge 2007 / SYSTOR 17, 2016)")
    add("      |  parse, validate, expand every request to 4KB blocks")
    add("      v")
    add("Normalized trace       timestamp,lba,size,operation,trace_id")
    add("      |  per-LBA gaps between consecutive writes")
    add("      v")
    add("Rewrite intervals      (first write of an LBA yields no target)")
    add("      |  sliding windows, chronological 70/15/15 split")
    add("      v")
    add("Sequences              L intervals -> the next interval")
    add("      |  pooled over pretraining workloads, targets held out")
    add("      v")
    add("Pretrained LSTM        models/pretrained/msr_pretrained.pt")
    add("      |  adapted on the target's EARLY training split only")
    add("      v")
    add("Fine-tuned LSTM        models/finetuned/*.pt")
    add("      |  scored on every predictable write")
    add("      v")
    add("Predictions            timestamp,lba,interval,class,model,trace")
    add("      |  threshold from TRAINING targets only")
    add("      v")
    add("Hot/cold labels        -> main log or cold log")
    add("      |")
    add("      v")
    add("C++17 LFS simulator    greedy cleaning, dedicated GC append point")
    add("      |  physical / logical bytes")
    add("      v")
    add("WAF, valid-block migrations, GC invocations")
    add("```")
    add("")
    ...
    if roles:
        add("## Workloads")
        add("")
        add("Roles assigned by the frozen rule in `ml/dataset_selection.py`, applied")
        add("before any model was trained and consulting no measured result.")
        add("")
        add(f"* **Pretraining** ({len(roles.get('pretrain', []))}): "
            f"{', '.join(roles.get('pretrain', [])) or 'none'}")
        add(f"* **Held-out targets** ({len(roles.get('targets', []))}): "
            f"{', '.join(roles.get('targets', [])) or 'none'}")
        add(f"* **External generalization** ({len(roles.get('external', []))}): "
            f"{', '.join(roles.get('external', [])) or 'none'}")
        if roles.get("excluded"):
            add(f"* **Excluded by eligibility**: {len(roles['excluded'])} workload(s); "
                "reasons in `dataset_inventory.csv`")
        add("")

    add("## Prediction quality on held-out test splits")
    add("")
    if model_comparison.empty:
        add("_`results/ml/model_comparison.csv` was not present when this was generated._")
    else:
        add("Lower MAE is better, higher F1 is better. `mae_log1p` is the scale the")
        add("model actually optimises and is far less dominated by a handful of very")
        add("long intervals than raw microseconds.")
        add("")
        add("| Trace | Predictor | Family | MAE (us) | RMSE (us) | MAE log1p | F1 | Accuracy |")
        add("| :--- | :--- | :--- | ---: | ---: | ---: | ---: | ---: |")
        for _, row in model_comparison.sort_values(["trace", "family", "predictor"]).iterrows():
            add(f"| {row['trace']} | {row['predictor']} | {row['family']} | "
                f"{row['mae_us']:,.0f} | {row['rmse_us']:,.0f} | {row['mae_log1p']:.4f} | "
                f"{row['f1']:.4f} | {row['accuracy']:.4f} |")
        add("")
        best = model_comparison.loc[model_comparison.groupby("trace")["mae_log1p"].idxmin()]
        add("Best log-space MAE per workload:")
        add("")
        for _, row in best.iterrows():
            add(f"* `{row['trace']}`: **{row['predictor']}** ({row['family']}), "
                f"MAE log1p {row['mae_log1p']:.4f}, F1 {row['f1']:.4f}")
        add("")

    add("## Write amplification by placement policy")
    add("")
    if ablation.empty:
        add("_`results/metrics/ablation.csv` was not present when this was generated._")
    else:
        add("Identical trace, geometry, working set and seed within each group; only")
        add("the placement decision differs. Negative `vs MIXED` is better.")
        add("")
        add("| Trace | Util | Configuration | WAF | vs MIXED | Valid migrations | GC bytes | Coverage |")
        add("| :--- | ---: | :--- | ---: | ---: | ---: | ---: | ---: |")
        for _, row in ablation.iterrows():
            change = ("baseline" if row["configuration"] == "MIXED"
                      else f"{row['waf_vs_mixed_pct']:+.2f}%")
            add(f"| {row['trace']} | {row['utilization_pct']:.0f}% | {row['configuration']} | "
                f"{row['waf']:.4f} | {change} | {int(row['valid_blocks_migrated']):,} | "
                f"{int(row['gc_bytes_copied']):,} | {row['prediction_coverage']:.1%} |")
        add("")

        others = ablation[ablation["configuration"] != "MIXED"]
        if not others.empty:
            best_row = others.loc[others["waf_vs_mixed_pct"].idxmin()]
            worst_row = others.loc[others["waf_vs_mixed_pct"].idxmax()]
            add(f"* Largest WAF reduction: **{best_row['waf_vs_mixed_pct']:+.2f}%** "
                f"({best_row['configuration']} on `{best_row['trace']}` at "
                f"{best_row['utilization_pct']:.0f}% utilization)")
            add(f"* Largest WAF increase: **{worst_row['waf_vs_mixed_pct']:+.2f}%** "
                f"({worst_row['configuration']} on `{worst_row['trace']}` at "
                f"{worst_row['utilization_pct']:.0f}% utilization)")
            add("")
    ...
    add("| File | Contents |")
    add("| :--- | :--- |")
    add("| `final_metrics.csv` | every simulator run with its change relative to MIXED |")
    add("| `model_metrics.csv` | prediction quality, models and baselines, same test splits |")
    add("| `classification_metrics.csv` | hot/cold confusion counts and derived rates |")
    add("| `dataset_inventory.csv` | measured statistics for every candidate workload |")
    add("| `experiment_manifest.json` | commit, config, seed, roles, caps, environment |")
    add("")
    add("Figures are in `results/plots/`; `docs/conference_demo.md` says what each one")
    add("is for. Methodology and the leakage audit are in `docs/methodology.md`.")
    add("")

    (FINAL_DIR / "README.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"  wrote {(FINAL_DIR / 'README.md').relative_to(REPO_ROOT)}")
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    write_final_readme(ablation, model_comparison)
    print("\nDone.")
    return 0
